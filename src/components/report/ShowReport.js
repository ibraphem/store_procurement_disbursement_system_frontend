...
const ShowReport = ({ company, from, to, action }) => {
  ...
  useEffect(() => {
    axios.get(`${URD}/${action}/${company}/${from}/${to}`).then((response) => {
      if (action === "Procurement") {
        setReporty(response.data.purchase);
            ...
            <div className="card">
              <div className="card-header">
                <h5 className={classes.tableTitle}>
                  <b>
                    {company === "Landover"
                      ? `${action.toUpperCase()} REPORT FOR LANDOVER COMPANY LIMITED FROM ${formatDate(
                          from
                        ).toUpperCase()} TO ${formatDate(to)}`
                      : `${action.toUpperCase()} REPORT FOR OVERLAND AIRWAYS LIMITED FROM ${formatDate(
                          from
                        ).toUpperCase()} TO ${formatDate(to)}`}
                  </b>
                </h5>
                  ...
                  <table className="table table-bord table-hover">
                    ...
                    <tbody>
                      ...
                      <tr className={classes.tableRow}>
                        <th className={classes.head} colSpan="6">
                          GRANT TOTAL
                        </th>
                        <th
