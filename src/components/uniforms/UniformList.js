...
const UniformList = () => {
  ...
  const handleRowAdd = (newData, resolve) => {
    ...
    if (newData.type === undefined) {
      errorList.push("Please enter uniform type");
      setIserror(true);
    }
    ...
    if (errorList.length < 1) {
      //no error
      axios
        .post(`${URD}/uniform/store`, newData)
        .then((response) => {
          setUniforms(response.data);
          resolve();
          setAlertMessage(["Uniform added successfully "]);
          setIserror(false);
        })
        .catch((error) => {
  ...
  const handleRowUpdate = (newData, oldData, resolve) => {
    let errorList = [];
    if (newData.type === "") {
      errorList.push("Uniform type can't be empty   ");
      setIserror(true);
    }

    if (errorList.length < 1) {
      axios
        .post(`${URD}/uniform/update/${oldData.id}`, newData)
        .then((response) => {
          setUniforms(response.data);
                  ...
                  detailPanel={[
                    {
                      tooltip: "Show",
                      render: (rowData) => {
                        return <Unistore uniform_id={rowData.id} />;
                      },
                    },
                  ]}
                />
